module EllieHelper
  def get_shopify_customers_full(params)
    Resque.logger.debug "EllieHelper#get_shopify_customers_full params: #{params}"
    option_value = params['option_value']
    uri = params['connection']
    sleep_shopify = params['sleep_shopify']
    Resque.logger.debug sleep_shopify
    Resque.logger.debug "sleep recharge: #{sleep_shopify}"
    ...
    if option_value == "full_pull"
      ...
      Resque.logger.info "We have #{num_customers} to download"
  ...
  def background_count_shopify_customers(shopify_header)
    ...
    Resque.logger.debug "EllieHelper#background_count_shopify_customers #{my_count}"
    return my_count
  end
  ...
  def background_count_yesterday_shopify_customers(my_yesterday, shopify_header)
    ...
    Resque.logger.debug "EllieHelper#background_count_yesterday_shopify_customers count: #{my_count}"
    return my_count
  end
  ...
  def background_load_full_shopify_customers(sleep_shopify, num_customers, shopify_header, uri)
    ...
    1.upto(num_pages) do |page|
      ...
      #Resque.logger.debug "#{'#' * 5} CUSTOMERS #{'#' * 40}\n#{customers.pretty_inspect}"
